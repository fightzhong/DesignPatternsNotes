...
### 引言
  ```
  对于序列化破坏单例的情况, 我们以饿汉模式进行分析, 其它模式的
  单例其实也是类似的, 到最后我们再统一总结, 首先看下面的代码先
  对序列化破坏有一个了解
  ```
...
### 总结
```
我们通过反序列化中反射创建的对象就不是原来的对象了, 所以我们
需要通过readResovle方法来防止这个出现, 从而破坏了单例模式,
同时也得记住在单例模式下的类如果需要序列化一定要进行防止破坏
```