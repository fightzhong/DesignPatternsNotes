...
### 引入
```
观察者模式描述的是一对多的依赖, 让多个观察者对象同时监听一个主题对象,
比如富二代xiaowang, 他有3个女朋友, xiaowang去哪里都带着他的三个女朋友,
于是他就会每天更新他自己的时间安排, 然后他的三个女朋友就会监听着他的安排,
从而跟着他去任何地方, 这里xiaowang就是主题对象, 他的三个女朋友就是观察者,
接下来我们用代码来演示一下
```
...
### 分析
```
这里小王通过notifyAllGrilFriend方法来触发更新, 从而让所有观察者收到
信息, 但是有一个弊端, 该方法内部使用循环遍历的方式来发送更新信息, 那么
该方法是一个同步方法, 所以存在高并发的情况
```

### 总结
```
定义: 定义了对象之间的一对多依赖, 让一对多依赖, 让多个观察者对象同时
      监听某一个主题对象, 当主题对象发生变化时, 它的所有依赖者(观察者)
      都会收到通知并更新
