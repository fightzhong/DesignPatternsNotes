...
### 引入
  ```
  状态模式和策略模式有点类似, 策略模式是通过将不同的算法传入主体中来达到
  不同的算法效果, 如超市就是主体, 超市执行不同的营销活动就是算法, 通过将
  不同的营销活动对象传入超市中, 从而让超市执行不同的营销活动, 再比如对一个
  数进行排序, 可以选择快速排序, 希尔排序, 冒泡排序等来达到排序的效果

  而状态模式是通过传入不同的状态来使得主体的表现形式不一样, 比如一个电视机,
  设置为播放状态, 则会播放电视, 设置为快速播放状态, 则会快速播放, 而状态之间
  可以进行转换, 如电视机本身处于播放状态, 然后就转换成暂停状态, 而由停止状态
  不能转换为暂停, 加速播放状态, 其实状态模式和策略模式的实现是很类似的, 下面
  我们通过代码来演示
  ```
  ...
  public class VideoPlayer {
    ...
    */
    ...
    public void play () {
      ...
      // 是假设此时电视机处于pause状态, 那么直接调用play功能, 就需要对状态进行转换, 即调用PauseState
      // 状态中的play方法来完成
      state.play( this );
    }
...
### 分析
  ```java
  一开始电视机处于Play状态, 所以调用play直接输出了正处于播放状态, 然后调用了
  pause方法, 由于当前状态处于PlayState, 所以调用该方法就是调用了PlayState中
  的pause方法, 即
  public void pause (VideoPlayer videoPlayer) {
    System.out.println( "电视机由播放状态切换为暂停状态" );
    videoPlayer.setState( VideoPlayer.PAUSE_STATE );
  ...
  所以会输出状态切换, 然后将videoPlayer的状态切换为Pause, 并执行pause方法,
  依次类推, 直到电视机处于stop状态时, 又执行了speed方法, 那么就会执行StopState
  中的speed方法, 即
  public void speed (VideoPlayer videoPlayer) {
    System.out.println( "停止播放状态不能切换为加速播放状态" );
  }
